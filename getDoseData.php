...
switch($action)
 {
   case "GetShowDose":
         ...
         $mandals_query = "SELECT d.`AwdId`,d.`AgeId`,d.`Dosage` ,dg.GroupType FROM dosage d
        LEFT JOIN `drugdetailsnew` n ON n.`DrugId`=d.`DrugId`
        LEFT JOIN druggroups dg ON dg.`GroupId`=n.`GroupId`WHERE n.`DrugId`=".$id." ORDER BY Dosage ASC limit 1;";
	...
	while($mandals_details=mysql_fetch_array($mandals_result))
	 { 
		...
		echo "<td>".$mandals_details["GroupType"]."</td>";
   ...
   case "GetMHUDrug":
		 ...
		 $mandals_query = "SELECT dm.`AwdId`,dm.`AgeId`,dm.`Dosage` ,dg.GroupType
FROM dosagemhu dm
LEFT JOIN `drugdetailsnewmhu` dnm ON dnm.`DrugId`=dm.`DrugId`
LEFT JOIN druggroups dg ON dg.`GroupId`=dnm.`GroupId` 
WHERE dnm.`DrugId`=".$id.";";
        $mandals_result=mysql_query($mandals_query);
