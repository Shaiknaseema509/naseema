...
<?php //echo "update medical_advice set question_id='".$ID."' where callid ='".$call_id."'";
  ...
  if($action == 'SAVEMEDICAL')
 {
	// echo "update medical_advice set summries_id='".$output."',advice='".$advice."',complaint='".$complaint."' where callid ='".$call_id."'";
	 mysql_query("insert into  medical_advice set dateTime=now(),agentId='".$agent_id."',remarks='".$remarks."',summries_id='".$output."',advice='".$advice."',complaint='".$complaint."',bskpoint='".$bskpoint."', callid ='".$call_id."'"); 
 ...
 if($action == 'SAVEMEDICALDOSAGES')
 {
	 ...
	 for($i=0;$i<$countUsage;$i++)
	 {
	  ...
	  `GroupId`='".$gId."',`DrugId`='".$gId."',`DrugName`='',`Dosage`='',`Description`='".$Groupdesc."',`callid`='".$call_id."',`dateTime`=NOW();"); 
