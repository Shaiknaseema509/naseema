...
var xmlHttp=newHttpObject();
	...
	if(spocname == "")	
	  ...
	  var callQuery="type=SaveBeneficiary&agent_id=<?=$agentID;?>&contact_no=<?=$phone_number;?>&spoccontact="+spoccontact+"&sEmail="+sEmail+"&Facility="+Facility+"&spocname="+spocname+"&city_name1="+city_name1+"&tehsil1="+tehsil1+"&rdate="+rdate+"&District="+District+"&call_id="+call_id+"&noc="+noc+"&designation1="+designation1+"&complaintagainst="+complaintagainst+"&callerEmail="+callerEmail+"&industryname="+industryname+"&landmark="+landmark+"&remarks_ro="+remarks_ro+"&goc="+goc+"&Compliant="+Compliant+"&Others="+Others+"&facility_name="+facility_name;
		//alert(callQuery);
...
<div class="col-md-12" style="color:white;background: radial-gradient(ellipse farthest-corner at center center) repeat scroll 0% 0%;background-color:#e1dfdf;border: 1px solid #eedcdc; "> 
	...
	<table 	class="tble tale-stiped ac" cellspacing="5" cellpadding="5">
		...
		<tr>
			...
			<td>			
			<select onchange="checkCompliant()" class="form-control custm_input_form" id="Compliant">
			...
			<div class="show-Others">
				<input style="width:90%" placeholder="Enter Others info" class="form-control custm_input_form custm_form_select" type="text" id="Others" />
			</div>
			</td>	
		...
		<td> <label class="custm_label_txt" style="padding-left: 165px;"><input type="checkbox" value="SMS" style="margin: 0px;" /> SMS </label></td>
