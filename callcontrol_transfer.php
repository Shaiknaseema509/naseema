...
$transfer_insert_q = "INSERT INTO inbound_transfer_details SET agent_id='".$agent_id."', action='".$action."', call_type='INBOUND', call_time=now(), lead_id='".$lead_id."', beneficiary_id='".$beneficiary_id."', call_id='".$call_id."',phone_number='".$phone_number."', process='".$process."', queue_name='".$_POST['tType']."', call_hit_referenceno='".$call_hit_referenceno."', queue_id='".$queue_id."', transfer_queue_name='".$tf_queue_name."', transfer_time=NOW()";
$transfer_insert_qLog = "INSERT INTO inbound_transfer_details SET agent_id='".$agent_id."', action='".$action."', call_type='INBOUND', call_time='".date('Y-m-d H:i:s')."', lead_id='".$lead_id."', beneficiary_id='".$beneficiary_id."', call_id='".$call_id."',phone_number='".$phone_number."', process='".$process."', queue_name='".$_POST['tType']."', call_hit_referenceno='".$call_hit_referenceno."', queue_id='".$queue_id."', transfer_queue_name='".$tf_queue_name."', transfer_time='".date('Y-m-d H:i:s')."'";
mysql_query($transfer_insert_q) or die(mysql_error());
dblog($transfer_insert_qLog);
        ...
        if($agent_status == "ONCALL")
         {
                ...
                $Query4 = "UPDATE call_incident_info SET call_end_time=NOW(),call_type_id='".$call_type_id."' WHERE callid='".$call_id."';";
               // mysql_query($Query4);
				dblog($Query4);
         }
		 
		 if($call_type_id != 9999 || $call_type_id != 35 || $call_type_id != 36 || $call_type_id != 39)
		 {
...
$CallTypeSSub             = $_POST["CallTypeSSub"];
	$Query5 = "UPDATE call_incident_info SET call_end_time=NOW(),call_type_id='".$call_type_id."', call_information_id='".$CallTypeSSub."',	beneficiary_id='1', popup_close_time=NOW() WHERE callid='".$call_id."';"; 
	$Query5Log = "UPDATE call_incident_info SET call_end_time='".date('Y-m-d H:i:s')."',call_type_id='".$call_type_id."', call_information_id='".$CallTypeSSub."',	beneficiary_id='1', popup_close_time='".date('Y-m-d H:i:s')."' WHERE callid='".$call_id."';"; 
	mysql_query($Query5);
