...
<?php //echo "update medical_advice set question_id='".$ID."' where callid ='".$call_id."'";
  ...
  if($action == 'SAVEMEDICAL')
 {
	// echo "update medical_advice set summries_id='".$output."',advice='".$advice."',complaint='".$complaint."' where callid ='".$call_id."'";
	 mysql_query("insert into  medical_advice set dateTime=now(),agentId='".$agent_id."',remarks='".$remarks."',summries_id='".$output."',advice='".$advice."',complaint='".$complaint."',bskpoint='".$bskpoint."',diagnosis='".$diagnosis."', callid ='".$call_id."'"); 
	 mysql_query("update medical_advice set dateTime=now(),agentId='".$agent_id."',remarks='".$remarks."',summries_id='".$output."',advice='".$advice."',complaint='".$complaint."',bskpoint='".$bskpoint."',diagnosis='".$diagnosis."' where callid ='".$call_id."'"); 
 }
 
 
 
 if($action == 'SAVEMEDICALDOSAGES')
 {
	 $call_id = $_POST['call_id'];
	 $countUsage = $_POST['countUsage'];
	 $remarks = $_POST['remarks'];
	 
	 for($i=0;$i<$countUsage;$i++)
	 {
		$GroupName =$_POST['groupType'][$i]; 
		$Groupdesc =$_POST['description'][$i]; 
		$gId =$_POST['gId'][$i]; 
		 if($GroupName == 'dosage') $gTYpes= 1;
		 else $gTYpes =2;
	  mysql_query("INSERT INTO `drugsdetailsdata` SET `remarks`='".$remarks."',`GroupName`='".$GroupName."',`GroupType`='".$gTYpes."',
	  `AwdId`='".$gId."',`DrugName`='',`Dosage`='',`Description`='".$Groupdesc."',`callid`='".$call_id."',`dateTime`=NOW();"); 
	  
	  //mysql_query("INSERT INTO `drugsdetailsdata` SET `remarks`='".$remarks."',`GroupName`='".$GroupName."',`GroupType`='".$gTYpes."',
	  //`AwdId`='".$gId."',`DrugId`='".$gId."',`DrugName`='',`Dosage`='',`Description`='".$Groupdesc."',`callid`='".$call_id."',`dateTime`=NOW();");
	
	 }
//	 mysql_query("update medical_advice set dateTime=now(),agentId='".$agent_id."',remarks='".$remarks."',summries_id='".$output."',advice='".$advice."',complaint='".$complaint."',bskpoint='".$bskpoint."' where callid ='".$call_id."'"); 
 }
