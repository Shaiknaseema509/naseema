		...
		if(xmlHttp)
		 {
			if(index == 1)
			 ...
			 {
				var cmo = document.getElementById("cmo_details").value;
				var cmo_details = cmo.split("~");
				var cmo_sno 	= cmo_details[0];
				var cmo_name	= cmo_details[1];
				var callQuery = "action=GetCMODetails&cmo_sno="+cmo_sno+"&cmo_name="+cmo_name;
			 }
					...
					if(index == 1)
					 {
						...
						document.getElementById("cmo_details").innerHTML=Response[1];
					 ...
					 {
						document.getElementById("cmo_contact_details").innerHTML=Response;
					//	document.getElementById("cmo_call_conf").style.display = '';	
					 }
			 	 }
		 	 }
	 	 }
                ...
                if(xmlHttp)
                 {
                        ...
                        var callQuery = "action=CLOSE&agent_id=<?=$_REQUEST["agentid"];?>&call_hit_referenceno=<?=$call_hit_referenceno;?>&call_type_id="+call_type_id_array[0]+"&beneficiary_id="+beneficiary_id;

			if((call_type_id_array[0] == 2)||(call_type_id_array[0] == 3)||(call_type_id_array[0] == 4)||(call_type_id_array[0] == 5)||(call_type_id_array[0] == 6))
	                 {
				 var call_id             = document.getElementById('callid').innerHTML;
				callQuery+="&Advice_sought_by="+Advice_sought_by+"&past_history="+past_history+"&present_compalint="+present_compalint+"&advice="+advice+"&call_id="+call_id+"&service_type="+call_type_id_array[0];
			 }
	...
	function conference_call()
	 {
		var xmlHttp=newHttpObject();
			...
			var emergency_type 	= document.getElementById("emergency_type").value;		
			if(emergency_type == "")
                         {
                                $('.alert').show();
                                $('.alert_content').html('Please Select EMERGENCY Type');
                                setTimeout(function(){$('.alert').hide();},10000); 
                                document.getElementById("emergency_type").focus();
                                return false;
                         }
			...
			var cmo	= document.getElementById("cmo_details").value;
	                if(cmo == "")
        	         {
                	        $('.alert').show();
                        	$('.alert_content').html('Please Select CMO');
	                        setTimeout(function(){$('.alert').hide();},10000); 
        	                document.getElementById("cmo_details").focus();
                	        return false;
                	 }
			var cmo_detail = cmo.split("~");
			var cmo_name   = cmo_detail[1];

			var cmo_phone_number = document.getElementById("cmo_contact_details").value;
	                if(cmo_phone_number == "")
        	         {
                	        $('.alert').show();
                        	$('.alert_content').html('Select CMO Contact');
	                        setTimeout(function(){$('.alert').hide();},10000); 
        	                document.getElementById("cmo_contact_details").focus();
                	        return false;
                	 }

                        var callQuery = "type=SaveCMODetails&call_id="+call_id+"&cmo_name="+cmo_name+"&emergency_type="+emergency_type+"&cmo_contact="+cmo_phone_number;
                        //alert(callQuery);//return false;
		...
		openWindowpostURL("http://<?=$GLOBALS["host_ip"];?>/ConVox3.0/Agent/control_panel.php?callid=<?=$call_id;?>&cmo_phone_number="+cmo_phone_number,"Conference_Call","width=325,height=486,left = 1000,top = 170,scrollbars=1,location=0, resizable=yes,dependant=yes,dialog=yes,modal=yes, unadorned=yes,status=0");
  	 }	
	...
	<div class="form-group" > 
        ...
        <div class="col-md-9" >	  
          <div class="row" >			 
				...
				<button type="button" name='recentBeneficiaryHistory' id='recentBeneficiaryHistory' class="btn-success"  onclick="openWindowpostURL('CallDetails.php?callid=<?=$transfer_callid;?>','Last_Adviced_Details','width=900px,height=400px,top=300px,left=200px,scrollbars=yes');">कॉल विवरण</buon>
				...
				<div class="col-md-3 blueclass " style="font-family:arial;font-size:15px;color:black;display:none;">विगत इतिहास :<span id="past_history_span" name="past_history_span"><input id="past_history" name="past_history" type="hidden" value="<?=$Beneficiary_Details['Past_history'];?>" style="font-family:arial;font-size:14px;color:black;width:5500px;height:40px;" class="col-md-12"></span></div>
				
				 <div class="col-md-3 blueclass " style="font-family:arial;font-size:15px;color:black;display:none;">वर्तमान शिकायत :<span id="present_complaint_span" name="present_complaint_span"><input type="hidden" id="present_complaint" name="present_complaint" value="<?=$Beneficiary_Details['present_compalint'];?>" style="font-family:arial;font-size:14px;color:black;width:5500px;height:40px;" class="col-md-12"></span></div> 	
					
				<div class="col-md-6 blueclass " style="font-family:arial;font-size:15px;color:black;display:none;">सलाह :<span id="advice_give_span" name="advice_span"><input id="advice" name="advice" type="hidden" value="<?=$Beneficiary_Details['advice'];?>" style="font-family:arial;font-size:14px;color:black;width:5500px;height:40px" class="col-md-12" ></span></div>
				
			<div class="col-md-3 blueclass " style="font-family:arial;font-size:15px;color:black;">	&nbsp;
                         	<span id="emergency_details" name="emergency_details">
                                <select id='emergency_type' name="emergency_type" class="col-md-12">
                                         <option value="">--Select Emergency Type--</option>
                                        <?
					mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
                                       $emer_type_query = "SELECT emergency_type_id,emergency_type_name FROM m_grievance_emergency_types WHERE is_active=1 ORDER BY order_by ASC";
                                        $emer_type_result= mysql_query($emer_type_query);
                                        while($emer_type_details = mysql_fetch_array($emer_type_result))
                                         {

                                                //$SEL = ($Beneficiary_Details['']==$emer_type_details[""])?"selected":"";
                                                echo "<option value='".$emer_type_details["emergency_type_id"]."~".$emer_type_details["emergency_type_name"]."' $SEL >".$emer_type_details["emergency_type_name"]."</option>";
                                         }
                                        ?>
                                        </select></span>
			 ...
			 <div class="col-md-3 blueclass " style="font-family:arial;font-size:15px;color:black;"> &nbsp;  
                                        <span id="cmo_span" name="cmo_span">
                                        <select id='cmo_details' name="cmo_details" class="col-md-12" onchange='GetRegions(this.value,"3");'>
                                         <option value="">-- Select CMO --</option>
                                       <?/*
					if($Beneficiary_Details['district_id'] !=''){
						mysql_query("SET character_set_results = 'utf8', character_set_client = 'utf8', character_set_connection = 'utf8', character_set_database = 'utf8', character_set_server = 'utf8'");
				        	$cmo_query = "SELECT officer_name,designation,office_number,mobile_number FROM Directory_cmo WHERE district_id =".$Beneficiary_Details['district_id']." ORDER BY officer_name ASC;";
						$cmo_result=mysql_query($cmo_query);
				        	while($cmo_details=mysql_fetch_array($cmo_result))
				       		  {
				                	echo "<option value='".$cmo_details["officer_name"]."~".$cmo_details["designation"]."' >".$cmo_details["officer_name"]."</option>";
						  }
					}*/
?>
					 </select></span>
                        ...
                        <div class="col-md-3 blueclass " style="font-family:arial;font-size:15px;color:black;"> &nbsp;
                                        <span id="cmo_details_span" name="cmo_details_span">
                                        <select id='cmo_contact_details' name="cmo_contact_details" class="col-md-12">
                                         <option value="">-- Select Contact --</option>
					</select>
			</div>
                      
			  <div class="col-md-3 blueclass " id='cmo_call_conf' style="font-family:arial;font-size:15px;color:black;display:none;"> &nbsp;
				<button type="button" name='cmo_conf_call' id='cmo_conf_call' class="btn-success" onclick="conference_call();">सीएमओ कॉल</button>
			</div>
			 ...
			 <div class="col-md-12" style="background-color: #E6EEFE ">
				...
				<?php
						//include('http://jansunwai.up.nic.in/IGRSAPP/INIT/login.aspx');
				
					?>				
				<a href="http://jansunwai.up.nic.in/IGRSAPP/INIT/login.aspx" target="myIframe">Click To Get Grievance Screen</a>
			 </div>
            <div class="col-md-12" >
