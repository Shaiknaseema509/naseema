...
//echo  $city_id;
   ...
   case 'GetEscalationDetails':
		...
		if($District ==''){ $district_array = explode("~",$district_id);$District = $district_array[0]; }
		
        $escalationboard       = $_POST['escalationboard'];  
			 
			 $call_type = "SELECT `Agency_id`,`Agency_Name`,`Mobile`,`email_id`,`designation` FROM `m_escalation_details` 
			 WHERE `District_id`=$District AND
`escalationBoardId`=$escalationboard AND `Level_No`=1 AND `isactive`=1 ;";
			$villages_detail = mysql_query($call_type);
