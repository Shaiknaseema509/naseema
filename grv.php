		...
		if(xmlHttp)
		 {
			if(index == 1)
			 ...
			 {
				var escalationboard = $('#escalationboard').val(); 
				var District = $('#District').val();  
				var callQuery = "action=GetEscalationDetails&escalationboard="+escalationboard+"&district_id="+District;
			 } 
	...
	/*if(noc == "")	
	  {
	    ...
	    var escalationboard	= document.getElementById('escalationboard').value;
	if(escalationboard == "")	
	  {
		showAlert();
		document.getElementById('escalationboard').focus();
		return false;
	  } 
	...
	if(spocname == "")	
	  {
	   ...
	   var callQuery="type=SaveBeneficiary&agent_id=<?=$agentID;?>&contact_no=<?=$phone_number;?>&call_id="+call_id+"&Compliant="+Compliant+"&goc="+goc+"&complaintagainst="+complaintagainst+"&Others="+Others+"&designation1="+designation1+"&noc="+noc+"&Facility="+Facility+"&industryname="+industryname+"&landmark="+landmark+"&rdate="+rdate+"&sector="+sector+"&tehsil1="+tehsil1+"&District="+District+"&city_name1="+city_name1+"&escalationboard="+escalationboard+"&spoccontact="+spoccontact+"&sEmail="+sEmail+"&spocname="+spocname+"&callerEmail="+callerEmail+"&Department="+Department+"&remarks_ro="+remarks_ro;
...
<div class="col-md-12" style="color:white;background: radial-gradient(ellipse farthest-corner at center center) repeat scroll 0% 0%;background-color:#e1dfdf;border: 1px solid #eedcdc; "> 
	...
	<table 	class="tble tale-stiped ac" cellspacing="5" cellpadding="5">
		...
		<tr>
			...
			<td rowspan="2" class="custm_label_txt"> Gist of Complaint</td>
			<td rowspan="2"><textarea class="form-control custm_input_form" id="goc" ></textarea></td>
		...
		<tr>			
			...
			<td style="display:none" class="custm_label_txt">Nature of Compliant</td>
		...
		<tr>
			...
			<td><input type="text" class="form-control datepicker custm_input_form" readonly  id="rdate" /></td>
			...
			<td>
			 <select id="sector" name="sector" class="form-control custm_form_input custm_form_select" style="width:95%">
		...
		<td class="custm_label_txt"> Village</td>
			...
			<td class="custm_label_txt">Escalation Board</td>
			<td> 
			<select class="form-control custm_input_form" id="escalationboard" onchange='GetRegions(this.value,99);' >
				<option value=''> Select Escalation </option>
				<?php $query = mysql_query("SELECT `ID`,`Name` FROM `m_escalation_boards` WHERE `ISACTIVE`=1;");
				while($db = mysql_fetch_array($query)){?>
				<option value='<?php echo $db['ID'];?>'><?php echo $db['Name'];?></option>
				<?php }?>
			</select>
			...
			<td><input type="text" id="sEmail" readonly class="form-control custm_input_form" />  </td>
			
			<td class="custm_label_txt"> SPOC Contact</td>
			<td><input type="text" id="spoccontact" readonly class="form-control custm_input_form" style="width: 160px;" />
	   ...
	   <tr>
		...
		<select id="facility_name" class="form-control" name="facility_name" style="display:none" onchange='GetRegions(this.value,"5");' >
			<option value=''> Select Facility </option></select>
  ...
  <script>
  $( function() {
    $( ".datepicker" ).datepicker({ yearRange: '2000:c',changeMonth: true,changeYear: true, maxDate: "+1D" });
  } );
  </script>
