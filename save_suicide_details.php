...
echo $_POST['referralsconcerned'];
		...
		$welfare_remarks	        = $_POST['welfare_remarks'];
 		$calltype_co					= explode("~",$_POST['calltype_co']);
...
switch($action)
{
   case 'SaveDetails': 
			...
			$Query123	= "INSERT INTO call_incident_info_details_suicide (`caller_id`,`patient_name`,`caller_name`,
						`alternate_number`,`age`,`gender_name`,`district_id`,`district_name`,`mandal_id`,`mandal_name`,`village_id`,`village_name`,`location`,phone_number,
						call_not_answered_reason) 
		...
		$Query1 = "UPDATE `call_incident_info_new` SET `call_type_id` = 32, `popup_close_time` = NOW() WHERE `callid` = '".$_POST['call_id']."';";
		mysql_query($Query1);
break;
...
`village_id` = '".$city_name1[0]."',`village_name` = '".$city_name1[1]."', `location` = '".$location."',`call_type_id` = '".$calltype_co[0]."', `category_id` = '".$category[0]."', `sub_category_id` = '".$subcategory[0]."', 
`risk_level_id` = '".$risklevel[0]."',  `referrals_concerned_name` = '".$referralsconcerned."', `co_remarks` = '".mysql_escape_string($co_remarks)."',
`consent_for_welfarecall` = '".$consent."',`agent_id` = '".$agent_id."',
 `status` = 1, `call_end_time` = NOW(), `call_not_answered_reason` = '".$notanswered."' 
 
 WHERE `caller_id` = '".$dba['caller_id']."'"; 
	...
	mysql_query($Query9); 
			...
			$Query12 = "UPDATE `call_incident_info` SET `call_type_id` = 32, `popup_close_time` = NOW() WHERE `callid` = '".$dba['caller_id']."';";
		mysql_query($Query12);
		dblog($Query12);
		
		$Query13 = "UPDATE `call_incident_info_new` SET `call_type_id` = 32, `popup_close_time` = NOW() WHERE `callid` = '".$dba['caller_id']."';";
		mysql_query($Query13);
 ...
 break;
			...
			$Query123	= "INSERT INTO `followup_not_answered` (`callid`,`call_not_answered_reason`,`call_not_answered_time`) VALUES ('".$_POST['caller_id']."','".$notanswered[0]."',NOW())";	
//echo "INSERT INTO `followup_not_answered` (`callid`,`call_not_answered_reason`,`call_not_answered_time`) VALUES ('".$_POST['caller_id']."','".$notanswered[0]."',NOW())";
			mysql_query($Query123);
			
						 
			$Query456	= "UPDATE `followup_save_details` SET `followup_call_not_answered_reason` ='".$notanswered[0]."' , 
			`call_not_answered_time` = NOW() WHERE `callid` = '".$_POST['caller_id']."'";
 			mysql_query($Query456);
			
			$Query321	= "DELETE FROM `followup_save_details` WHERE callid = '".$caller_id."' AND `followup_number` = '".$_POST['followup_number']."' ";	
//echo "DELETE FROM `followup_save_details` WHERE callid = '".$caller_id."' AND `followup_number` = '".$_POST['followup_number']."'";
			mysql_query($Query321);
 ...
 break;
			...
			$Query123	= "DELETE FROM `followup_save_details` WHERE callid = '".$caller_id."' AND `followup_number` = '".$_POST['followup_number']."' ";	
			echo "DELETE FROM `followup_save_details` WHERE callid = '".$caller_id."' AND `followup_number` = '".$_POST['followup_number']."'";
			mysql_query($Query123);
